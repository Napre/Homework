...
//Establish arange of values on the complex plane
...
//Start at nagative half the width and height
float xmin=-w/2;
float ymin=-h/2;

//Make sure we can write to the pixels[]array
//Only need to do this once sinse wo don't do any other drawing.
loadPixels();
...
for(int j=0;j<height;j++){
  ...
  for(int i=0;i<width;i++){
    ...
    //Now we test,as we interate z=z^2+c does z tend towards infinity?
    float a=x;
    float b=y;
    int n=0;
    float max=4.0;//Infinity in our finite world is simple,let's just it 4
    float absOld=0.0;
    ...
    //If we never got there,let's pick the color balck
    if(n==maxiterations){
